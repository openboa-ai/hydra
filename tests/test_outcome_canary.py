...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_counts_nested_values_in_section_mapping(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
source, target = map(Path, sys.argv[1:])
try:
    by_kind = {item["kind"]: item["value"] for item in map(json.loads, source.read_text().splitlines())}
except (KeyError, json.JSONDecodeError):
    raise SystemExit(2)
values = [by_kind["outcome"], by_kind["evidence"], by_kind["unknown"]]
lines = []
for index, title in enumerate(("Outcome", "Evidence", "Unknowns")):
    lines.extend([f"# {title}", f"- {values[index]}"])
    lines.extend(f"  - {value}" for offset, value in enumerate(values) if offset != index)
target.write_text("\\n".join(lines))
""",
                encoding="utf-8",
            )
        ...
        self.assertEqual(1, completed.returncode)
        self.assertIn(
            "section-value-separation", json.loads(completed.stdout)["failed_checks"],
        )

    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_ignores_values_inside_html_comments(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
try:
    ...
    raise SystemExit(2)
target.write_text("\\n".join([
    "# Outcome", "<!--", f"- {by_kind['outcome']}", "-->",
    "# Evidence", "<!--", f"- {by_kind['evidence']}", "-->",
    "# Unknowns", "<!--", f"- {by_kind['unknown']}", "-->",
]))
""",
                encoding="utf-8",
            )
        ...
        self.assertIn("outcome-preservation", failed)
        self.assertIn("evidence-preservation", failed)
        self.assertIn("unknown-preservation", failed)

    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_indented_code_as_list_values(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_values_inside_raw_html_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
        ...
        self.assertEqual(1, completed.returncode)
        self.assertIn(
            "section-separation", json.loads(completed.stdout)["failed_checks"],
        )

    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_dangling_malformed_output_symlink(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
        ...
        self.assertEqual(1, completed.returncode)
        self.assertIn("malformed-input", json.loads(completed.stdout)["failed_checks"])

    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_processing_instruction_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_nbsp_fence_closer(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_nbsp_list_separator(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_comment_prefixed_markers(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
prefix = "<!---->"
target.write_text("\\n".join([
    prefix + "# Outcome", prefix + f"- {by_kind['outcome']}",
    prefix + "# Evidence", prefix + f"- {by_kind['evidence']}",
    prefix + "# Unknowns", prefix + f"- {by_kind['unknown']}",
]))
""",
                encoding="utf-8",
            )
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_excess_list_padding(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_varies_event_order_for_kind_routing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
...
source, target = map(Path, sys.argv[1:])
try:
    values = [item["value"] for item in map(json.loads, source.read_text().splitlines())]
except (KeyError, json.JSONDecodeError):
    raise SystemExit(2)
target.write_text("\\n".join([
    "# Outcome", f"- {values[0]}",
...
""",
    ...
    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_rejects_intermediate_entrypoint_symlink(self) -> None:
        with tempfile.TemporaryDirectory() as directory, tempfile.TemporaryDirectory() as external:
            root = Path(directory)
            outside = Path(external)
            (outside / "handoff.py").write_text("raise SystemExit(0)\n", encoding="utf-8")
            (root / "tools").symlink_to(outside, target_is_directory=True)
            completed = subprocess.run(
                [
                    sys.executable, str(ROOT / "scripts/run_outcome_canary_blackbox.py"),
                    "--candidate-root", str(root), "--entrypoint", "tools/handoff.py",
                ],
                text=True, capture_output=True, check=False,
            )
        self.assertEqual(1, completed.returncode)
        self.assertIn("ValueError", json.loads(completed.stdout)["failed_checks"])

    @unittest.skipUnless(ACTION_HARNESS_AVAILABLE, "requires non-root Linux GitHub Actions")
    def test_trusted_blackbox_ignores_bullets_inside_fences(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "handoff.py").write_text(
                """import json
