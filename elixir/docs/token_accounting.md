...
This document explains how Codex reports token usage through the app-server protocol and how Hydra should account for it.
...
Hydra sees these events wrapped inside the app-server message stream.
...
For Hydra, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Hydra
...
## What Hydra Should And Should Not Do
...
- Expect one thread to span multiple turns when Hydra reuses a live Codex thread.
...
## Practical Interpretation For Hydra Logs
...
That is a strong signal for Hydra:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Hydra Documentation Contract

If Hydra documents token reporting externally, the contract should be:

- Live token totals come from Codex thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Hydra does not use it for totals.
