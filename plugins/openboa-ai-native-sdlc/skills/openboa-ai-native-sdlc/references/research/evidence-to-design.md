...
| D-17 | OAI-07, OAI-09 | Hooks and scheduled tasks are distinct native wakeup surfaces with different context, trust, persistence, and sandbox behavior. | **Adopted and narrowed in v0.2:** use read-only SessionStart and PostCompact diagnostics; choose scheduled tasks only after live capability discovery; keep scheduled tasks read-only and route mutations to interactive work; bind wakeup, expiry, notification, and stop conditions. | Hook tests prove no mutation or network action. Automation contracts reject scheduled checkout and external writes and cases cover unavailable capability, no-change wakeups, uncertain effects, and persistent-install gates. |
| D-18 | OAI-08, OAI-10, STD-02, STD-04 | Non-interactive execution is useful only when packaging, sandbox, provenance, lifetime containment, locks, and authority remain explicit. | **Narrowed in v0.2 after hostile review:** do not package a generic local runner, launchd job, or cron entry. A descendant can detach from a portable process group, so unattended writes require a future environment-specific containment adapter. Use native Codex scheduled tasks, GitHub Actions, or interactive worktrees instead. | Hostile review reproduced same-group and detached descendants. Contract tests require the unsupported boundary and absence of the unsafe runner assets. |
