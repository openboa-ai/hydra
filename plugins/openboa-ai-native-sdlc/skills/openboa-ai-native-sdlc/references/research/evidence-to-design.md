...
| D-18 | OAI-08, OAI-10, STD-02, STD-04 | Non-interactive execution is useful only when packaging, sandbox, provenance, locks, and authority remain explicit. | **Adopted in v0.2:** package a one-shot headless runner with read-only default, approval never, clean isolated worktree requirement for writes, JSONL evidence, lock, timeout, and no full-access mode. | Hostile tests cover relative paths, dirty primary checkout, prompt leakage, lock behavior, timeout, and exact installed bytes. |
