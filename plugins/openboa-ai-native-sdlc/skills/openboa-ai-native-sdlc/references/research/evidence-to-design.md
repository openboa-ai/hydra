...
| D-17 | OAI-07, OAI-09 | Hooks and scheduled tasks are distinct native wakeup surfaces with different context, trust, persistence, and sandbox behavior. | **Adopted in v0.2:** use read-only SessionStart and PostCompact diagnostics; choose scheduled tasks only after live capability discovery; bind wakeup, expiry, notification, and stop conditions. | Hook tests prove no mutation or network action. Automation cases cover unavailable capability, no-change wakeups, uncertain effects, and persistent-install gates. |
