...
Hydra now prepares that canary without changing the live ruleset: `.github/openboa-governance.yml` lists control-plane paths, `scripts/validate_governance.py` runs from the trusted source checkout and reads the base and candidate only as data, and `.github/workflows/openboa-governance-v2.yml` keeps all three checkouts separate. The risk lane is computed from the event-recorded base and head revisions, so a later `main` advance cannot turn unrelated changes into candidate risk. Until a human-gated ruleset selects the trusted repository, branch, and workflow file, `openboa-governance-v2` is bootstrap evidence rather than a trusted merge gate. Keep the existing `openboa-governance` context in place through the transition and require an exact-head live readback before renaming or removing anything.
