...
name: debug
description:
  Investigate stuck runs and execution failures by tracing Hydra and Codex
...
- Primary runtime log: `log/hydra.log`
  - Default comes from `SymphonyElixir.LogFile` (`log/hydra.log`).
  - Includes orchestrator, agent runner, and Codex app-server lifecycle logs.
- Rotated runtime logs: `log/hydra.log*`
...
rg -n "issue_identifier=MT-625" log/hydra.log*

# 2) If needed, narrow by Linear UUID
rg -n "issue_id=<linear-uuid>" log/hydra.log*

# 3) Pull session IDs seen for that ticket
rg -o "session_id=[^ ;]+" log/hydra.log* | sort -u

# 4) Trace one session end-to-end
rg -n "session_id=<thread>-<turn>" log/hydra.log*

# 5) Focus on stuck/retry signals
rg -n "Issue stalled|scheduling retry|turn_timeout|turn_failed|Codex session failed|Codex session ended with error" log/hydra.log*
```
...
In Hydra, Codex session diagnostics are emitted into `log/hydra.log` and
...
- Check rotated logs (`log/hydra.log*`) before concluding data is missing.
